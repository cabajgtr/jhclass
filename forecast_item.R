...
##
##
item <- TOYS[#SEGMENT4=="MY PAL SCOUT" # & TT_CUSTOMER == "WMT" ##May want to reduce items to forecast
     ,.(units = sum(units))
...
setkey(item,SEGMENT4,POS_YEAR,POS_MTH_NBR)
setkey(item.test,SEGMENT4,POS_YEAR,POS_MTH_NBR)
item.acc <- (item[item.test][UNITS_FC!=0 & POS_YEAR == 2014])
item.acc.summary <- 
     item.acc[,z<-(.SD),by=.(SEGMENT4),.SDcols = c("UNITS_FC","units")]
...
forecast_table <- function(x) {
     ...
     #Check for critical length of history
          ...
          if(as.yearmon(tend.dt) - as.yearmon(tstart.dt) < 2) {
...
acc <- function(x){ #expects data.frame or similar with col1 = Forecast, Col2 = Actuals
     f <- as.data.frame(item.acc)[,1]
     a <- as.data.frame(item.acc)[,2]

     acc <- tryCatch(
          accuracy(f,a)
          , error = function(e) exit(NULL)
     )
     return(
          cbind(round(acc,2)
